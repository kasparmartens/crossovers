...
Note that the labelling of the latent states is arbitrary, i.e. the model specification is invariant to relabelling the states. Here we relabel the latent states of each sample, to obtain a close match to the true values in $\boldB$, i.e. for each row $i$ of the transition matrix $\boldB$ we find the state $j$ such that the current sample $\boldB^{*}_{i, :}$ would be closest to $\boldB_{j, :}$ in terms of KL divergence. 
